...
\begin{abstract}
We organize two elementary and exact cancellation mechanisms for quadratic-character sums attached to Lucas sequences modulo an inert odd prime. The first is translation by a matrix half-period, yielding the anti-periodicity relation $U_{n+T/2}=-U_n$. The second is rank-block scalar propagation, equivalently a geometric character factorization, in which the multiplier $\Lambda^k$ filters complete character sums. These mechanisms are logically distinct as structural arguments: the former is an odd-function pairing under index translation, whereas the latter is multiplicative filtering by the rank scalar. The determinant/norm relation links the companion-matrix determinant to the field norm of a characteristic root and forces $\ord(Q)\mid T$; it is kept as an independent observation and is not itself a cancellation criterion. In the quadratic-character full-period setting the two criteria are nevertheless related: the half-period criterion implies the rank-block scalar criterion, while the converse need not hold. No new rank--period theory is claimed.
\end{abstract}
...
\cite{FiebigMbirikaSpilker2025} study the period, entry-point, and order
statistics of Lucas sequences modulo $m$, including antipodal
additive-inverse relations when $q=-1$. The present note is complementary:
in the inert prime setting, the companion-matrix and quadratic-extension
structure yields cancellation criteria for character sums, a level of
structure beyond the statistics of the period alone. For the quadratic character on a full matrix period the two criteria are not independent: by \cref{prop:chi-lambda}, the half-period criterion always implies the scalar criterion, while the converse need not hold.
...
\]
Since $A^T=I$, the sequences $(U_n)_{n\geq0}$ and $(V_n)_{n\geq0}$ are periodic modulo $p$ with period $T$.
...
The identity $A^\alpha=\Lambda I$ and the decomposition \eqref{eq:period-decomposition} are the inert-case form of the classical relation between the period, the rank of apparition, and the order of the multiplier for Fibonacci and Lucas sequences \cite{Robinson1963,Vinson1963,Wall1960,Renault1996,Renault2013}. The divisibility $\alpha\mid p+1$ of \cref{cor:alpha-divides-p-plus-1} is classical as well \cite{Lucas1878,Wall1960,GuptaRockstrohSu2012}. Taking determinants in $A^\alpha=\Lambda I$ also yields the norm identity $\Lambda^2=Q^{\alpha}$, which links the rank multiplier to \cref{rem:determinant-norm}. 
We note that \cite{FiebigMbirikaSpilker2025} deliberately develop their
theory without using the interpretation of the order statistic as the
multiplicative order of the rank multiplier; in the inert setting, this
interpretation, together with the scalar propagation of
\cref{prop:rank-multiplier}, is precisely what yields the geometric
character factorization of \cref{prop:full-period-factorization}.
\end{remark}
...
For the quadratic character on a full matrix period, \cref{prop:chi-lambda} nevertheless shows that the two criteria are related: the half-period criterion implies the scalar criterion, while the converse need not hold. Moreover, since a nonsquare in the cyclic group $\Fp^\times$ has even multiplicative order (see the proof of \cref{prop:chi-lambda}(b)), the condition $\chiP(\Lambda)=-1$ forces $\omega$, and hence $T=\alpha\omega$, to be even, so that $A^{T/2}=-I$ by \cref{prop:half-period-criterion}. Anti-periodicity is thus always present when the scalar criterion is active; whether it produces quadratic-character cancellation still depends on $\chiP(-1)$.
\end{remark}
...
The author discloses that artificial intelligence was used for linguistic editing, organizational composition, notation normalization; the author independently verified mathematical expressions, examples, numerical checks, citations, and reproducibility claims, and takes full responsibility for the final version of the article. The author's ORCID is \href{https://orcid.org/0009-0001-1097-1770}{0009-0001-1097-1770}.
