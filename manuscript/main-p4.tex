...
\[
 p\nmid QD
\]
and $\chiP(D)=-1$, and all congruences in what follows are taken modulo $p$.
...
\toprule
& $p$ & $T$ even? & $\chiP(-1)$ & $\chiP(\Lambda)$ & HP quadratic & Scalar \\
\midrule
\cref{ex:fibonacci-7} & $7$ & yes & $-1$ & $-1$ & yes & yes \\
\cref{ex:scalar-cancellation} & $5$ & yes & $1$ & $-1$ & no & yes \\
\cref{rem:even-period-not-enough} & $5$ & yes & $1$ & $1$ & no & no \\
\bottomrule
\end{tabular}
\caption{The three configurations of the two quadratic criteria realized by the
examples. The column ``HP quadratic'' refers to the quadratic cancellation criterion of
\cref{cor:quadratic-half-period}, not merely to the existence of $A^{T/2}=-I$. The fourth
formal configuration (half-period quadratic criterion active, scalar criterion inactive)
is excluded by \cref{prop:chi-lambda}.}
