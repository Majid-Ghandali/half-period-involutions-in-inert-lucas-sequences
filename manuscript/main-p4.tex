...
\[
 D=P^2-4Q.
\]
Although Lucas sequences are often studied under characteristic-zero non-degeneracy assumptions, no such global assumption is needed for the finite-field arguments below. Throughout we assume only the local conditions
\[
 p\nmid QD
\]
...
\toprule
& $p$ & $\chiP(-1)$ & $\chiP(\Lambda)$ & HP quadratic & Scalar \\
\midrule
\cref{ex:fibonacci-7} & $7$ & $-1$ & $-1$ & yes & yes \\
\cref{ex:scalar-cancellation} & $5$ & $1$ & $-1$ & no & yes \\
\cref{rem:even-period-not-enough} & $5$ & $1$ & $1$ & no & no \\
\bottomrule
\end{tabular}
